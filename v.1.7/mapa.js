...
function mostrarMapa() {
    ...
    const btnGestionarUsuarios = document.getElementById('btn-ver-usuarios');
    
    if (usuarioActual && usuarioActual.rol === 'admin') {
        btnGestionarUsuarios.style.display = 'block'; // Se le muestra solo al admin
    } else {
        btnGestionarUsuarios.style.display = 'none'; // Se le oculta al usuario normal
    }
...
async function cambiarRolUsuario(idUsuario, rolActual) {
    // Definimos el nuevo rol de forma opuesta al actual
    ...
    try {
        // Hacemos la petición PUT al backend para actualizar la base de datos
        const respuesta = await fetch(`https://waynorte-backend.onrender.com/api/usuarios/${idUsuario}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
        ...
        if (respuesta.ok) {
            ...
            // Recargamos la tabla automáticamente para ver los cambios reflejados en tiempo real
            CargarTablaUsuariosAdmin();
        } else {
            const datosError = await respuesta.json();
            alert(`Error del servidor: ${datosError.error || 'No se pudo cambiar el rol'}`);
        }
    } catch (error) {
...
function abrirModalUsuarios() {
    ...
    cargarTablaUsuariosAdmin(); // Pedimos los datos cada vez que se abre
}

// 2. Función para cerrar desde la X
function cerrarModalUsuarios() {
    document.getElementById('modal-usuarios').style.display = 'none';
}

// 3. Función que pide los usuarios a tu backend y rellena la tabla
async function cargarTablaUsuariosAdmin() {
    const tbody = document.getElementById('lista-usuarios-body');
    tbody.innerHTML = '<tr><td colspan="4">Cargando usuarios...</td></tr>';

    try {
        // --- REEMPLAZA ESTO CON TU LLAMADA AL SERVIDOR O FIREBASE ---
        // Ejemplo con fetch a tu propio backend:
        // const response = await fetch('/api/usuarios');
        // const usuarios = await response.json();
        
        // Datos de prueba para que veas cómo funciona el diseño:
        const usuarios = [
            { id: 1, nombre: 'Admin', email: 'admin@test.cl', rol: 'admin' },
            { id: 2, nombre: 'Juan Pérez', email: 'juan@test.cl', rol: 'usuario' },
            { id: 3, nombre: 'María Silva', email: 'maria@test.cl', rol: 'usuario' }
        ];
        // -------------------------------------------------------------
        ...
        usuarios.forEach(user => {
            ...
            tr.innerHTML = `
                <td>${user.nombre}</td>
                <td>${user.email}</td>
                <td><span class="badge-${user.rol}">${user.rol}</span></td>
                <td>
                    <button onclick="cambiarRol(${user.id}, '${user.rol}')">
                        ${user.rol === 'admin' ? 'Hacer Usuario' : 'Hacer Admin'}
                    </button>
                    <button style="color: red;" onclick="eliminarUsuario(${user.id})">
                        Eliminar
                    </button>
                </td>
            `;
